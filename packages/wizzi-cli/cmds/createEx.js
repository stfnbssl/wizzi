...
module.exports = (options) => {
    ...
    wizzi.model(path.join(templatorContextPath), {
        ...
        file.deleteFolder(tempTemplate, (err) => {
        
            if (err) {
                console.log("[31m%s[0m", 'createEx.delete temp template folder.error:', err);
                return ;
            }
            ...
            if (options.template == "wizzi/plugin") {
                ...
                cliCtx.fileCtx = fileCtx;
            }
            
            console.log('createEx.before genFolder.cliCtx', cliCtx, __filename);
            
            return wizzi.genFolder(tempTemplate, cliCtx, {
