/*
    ...
    primary source IttfDocument: c:\my\wizzi\v5\apps\wizzi-demo\src\ittf\examples\advanced\essentials\filesystem\jobs\mongodb\level_0\step_1_upload.js.ittf
...
var wizzi_job_mongodb_level_0_step_1_upload = function(step_callback) {
    ...
    repo.mongoDbDocumentManager(testMongodbUri, function(err, docman) {
        ...
        docman.uploadFolder(path.join(__dirname, 'step_1'), path.join(demoUploadMongoBaseFolder, 'wizzijob', 'level_0', 'step_1'), function(err, result) {
            if (err) {
                throw new Error(JSON.stringify(err, null, 2));
            }
