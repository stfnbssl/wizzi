/*
    ...
    primary source IttfDocument: C:\My\wizzi\v5\apps\wizzi-demo\src\ittf\examples\advanced\essentials\filesystem\schemas\level_0\step_2_go.js.ittf
*/
'use strict';
...
var wizzischema = function(step_callback) {
    ...
    */
    ...
    wizzi.fsFactory({
        ...
        // The wizzi model artifact "sample_0_2" is created after that the wizzi factory
        ...
        wf.generateModelDoms(path.join(__dirname, 'step_2', 'sample_0_2.wfschema.ittf'), path.join(__dirname, 'local'), 'sample_0_2', mTreeBuildupContext, function(err, result) {
            if (err) {
                var fullErr = JSON.stringify(err, null, 2);
                console.log('Error fullErr', fullErr);
                throw new Error(JSON.stringify(err, null, 2));
            }
            printValue('Result', 'Wizzi model of schema sample_0_2 generated. \n' + JSON.stringify(result, null, 2), 'dashes');
            wf.loadModel(path.join(__dirname, 'step_2', 'first.sample_0_2.ittf'), mTreeBuildupContext, function(err, wizziModel) {
                if (err) {
                    var fullErr = JSON.stringify(err, null, 2);
                    console.log('Error fullErr', fullErr);
                    throw new Error(JSON.stringify(err, null, 2));
                ...
                printValue('Result', 'Ittf document first.sample_0_2.ittf loaded.', 'dashes');
...
function heading1(text) {
    ...
    console.log('** level 0 - step 2 - wizzischema - ' + text);
    console.log('*'.repeat(120));
    console.log('');
}
