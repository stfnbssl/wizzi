/*
    ...
    primary source IttfDocument: C:\My\wizzi\stfnbssl\wizzi\packages\wizzi-meta\.wizzi\ittf\examples\wfpackage\index.js.ittf
*/
'use strict';
//
// Example skeleton specific for the 'wizzi-meta' plugin package
//
//
...
var Meta_wfpackage_step_1 = function(step_callback) {
    heading1('EXAMPLE')
    heading2('Meta wfpackage - started')
    loadAndTransformModel(path.join(__dirname, 'ittf', 'models', 'wizzi.kernel.wfpackage.ittf'), {}, "wfpackage/fs-scanner", function(err, transformed) {
        if (err) {
            console.log('Test error >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>');
            console.log('err', err);
            throw new Error(err.message);
        }
        file.write(path.join(__dirname, 'outputs', 'wizzi.kernel.package.json'), stringify(transformed, null, 2))
        heading2('Meta wfpackage - ended, see outputs folder')
        code_proprocess_job();
        // _ site_job
    })
    function code_proprocess_job() {
        heading2('codeviewer_preprocess job - started')
        executeWizziJob(path.join(__dirname, 'ittf', 'jobs', 'codeviewer_preprocess.wfjob.ittf'), {
            jobCtx: {
                kind: 'kernel'
             }
            ...
            heading2('codeviewer_preprocess job - ended')
            code_job();
        })
    }
    function code_job() {
        heading2('codeviewer job - started')
        executeWizziJob(path.join(__dirname, 'ittf', 'jobs', 'codeviewer.wfjob.ittf'), {
            jobCtx: {
                kind: 'kernel'
             }
         }, function(err, jobResult) {
            ...
            heading2('codeviewer job - ended')
            site_job();
        })
    }
    function site_job() {
        heading2('site job - started')
        executeWizziJob(path.join(__dirname, 'ittf', 'jobs', 'site.wfjob.ittf'), {
            jobCtx: {
                kind: 'kernel'
             }
         }, function(err, jobResult) {
            ...
            heading2('site job - ended')
        })
    }
};
Meta_wfpackage_step_1.__name = 'Meta_wfpackage_step_1';
function heading1(text) {
    console.log('');
    console.log('*'.repeat(120));
    console.log('** level 0 - step 2 - Meta_wfpackage_step_1 - ' + text);
    console.log('*'.repeat(120));
    console.log('');
}
function heading2(text) {
    ...
    console.log('   ','-- Meta_wfpackage_step_1 - ' + text);
    console.log('   ', '-'.repeat(100));
    console.log('');
}
...
module.exports = Meta_wfpackage_step_1;
if (typeof require != 'undefined' && require.main === module) {
    Meta_wfpackage_step_1();
}
