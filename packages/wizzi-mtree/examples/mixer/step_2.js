/*
    ...
    package: wizzi-js@
    primary source IttfDocument: C:\My\wizzi\stfnbssl\wizzi\packages\wizzi-mtree\.wizzi\examples\mixer\step_2.js.ittf
...
var Mixer_Step_1 = function(step_callback) {
    heading1('EXAMPLE')
    var ittfUri = path.join(__dirname, 'ittf', 'schemas', 'wfschema.wfschema.ittf');
    getFSDocumentStore(function(err, fsStore) {
        ...
        MTreeBrickProvider.createFromUri(ittfUri, {
            ...
            printNodes(mTree.nodes, 'Before mix');
            includer(mTree, provider, function(err, includedModel) {
                ...
                mixer(includedModel, provider, function(err, mixedModel) {
                    if (err) {
                        console.log("[31m%s[0m", 'err', err);
                        if (err.toString()) {
                            console.log("[31m%s[0m", 'err.toString()', err.toString());
                    ...
                    printNodes(mixedModel.nodes, 'After mix');
                })
            })
        })
    })
};
Mixer_Step_1.__name = 'Mixer_Step_1';
function heading1(text) {
    console.log('');
    console.log('*'.repeat(120));
    console.log('** level 0 - step 1 - Mixer_Step_1 - ' + text);
    console.log('*'.repeat(120));
    console.log('');
}
