...
var ContextData = (function () {
    ...
    */
    ContextData.prototype.enterFunction = function(functionName) {
        ...
        assert(this.ittfState == 1 || this.ittfState == 2, 'ContextData: Method `enterFunction` must be called on ittfStates 1 or 2 only')
    ...
    */
    ContextData.prototype.exitFunction = function(functionName, restoreFunctionName) {
        ...
        // throw new("Here it is")
        if (functionName == "buildNode" && ctx.functionsStack.length == 1) {
        }
    ...
    */
    ContextData.prototype.getContextOfDeclared = function(name) {
        console.log('ContextData.getContextOfDeclared', name, 'ittfState', this.ittfState, 'brickKey', this.brickKey, 'currentFunctionName', this.currentFunctionName,);
        if (!this.currentFunctionName) {
            console.log('ContextData.getContextOfDeclared', name, 1, this.isDeclaredInScope(name));
            return this.isDeclaredInScope(name) ? this : null;
        }
        else {
            var ctx = this.getContextOf(this.currentFunctionName);
            if (ctx.isDeclaredInScope(name)) {
                console.log('ContextData.getContextOfDeclared', name, 2, this.currentFunctionName);
                return ctx;
            }
            ...
            while (parentScopeFunction) {
                ...
                if (ctx.isDeclaredInScope(name)) {
                    console.log('ContextData.getContextOfDeclared', name, 3, this.parentScopeFunction);
                    return ctx;
                }
                parentScopeFunction = this.getParentScopeFunction(parentScopeFunction);
            }
        }
        console.log('ContextData.getContextOfDeclared', name, 4, this.isDeclaredInScope(name));
        return this.isDeclaredInScope(name) ? this : null;
    }
    ...
    ContextData.prototype.put = function(name, value) {
        ...
        if (IsV09) {
            console.log('ContextData.put', name, value, 'currentFunctionName', this.currentFunctionName);
            var ctx = this.getContextOfDeclared(name);
            if (ctx) {
                ctx.values[name] = value;
                console.log('ContextData.put', name, value, 'ctx.values', ctx.values, 'ctx.valuesStack.length', ctx.valuesStack.length, 'this.currentFunctionName', this.currentFunctionName, 'ctx.brickKey', ctx.brickKey);
                return true;
            }
            else {
                return false;
    ...
    ContextData.prototype.getValue = function(name) {
        ...
        if (IsV09) {
            if (!this.currentFunctionName) {
                ...
                if (v != undefined) {
                    console.log('ContextData.getValue 2', name, v);
                    return v;
                }
