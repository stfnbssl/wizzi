/*
    ...
    package: wizzi-js@
    primary source IttfDocument: C:\My\wizzi\stfnbssl\wizzi\packages\wizzi-mtree\.wizzi\tests\all\loader\evaluator_with_params.js.ittf
...
describe("evaluator", function() {
    ...
    it("should load and evaluate", function(done) {
        var content_filepath = path.join(__dirname, 'repo', 'data', 'evaluator_1_with_params.tests.ittf');
        var loadContext = {
            mTreeBuildUpContext: {}, 
            productionContext: mocks.ProductionContext, 
            __ittfDocumentStore: store
    ...
    it("should load and evaluate nodes[0]", function() {
    ...
    it("should load and evaluate nodes [0][0]", function() {
        ...
        expect(node.c).to.be(1);
        expect(node.n).to.be.a('string');
        expect(node.n).to.be('tau');
        expect(node.v).to.be.a('string');
        expect(node.v).to.be('10');
    });
    it("should load and evaluate nodes [0][0][0]", function() {
        node = evaluatedModel.nodes[0].children[0];
        expect(node.children).to.be.an('array');
        expect(node.children.length).to.be(1);
        node = node.children[0];
        expect(node.r).to.be.a('number');
        expect(node.r).to.be(3);
        expect(node.c).to.be.a('number');
        expect(node.c).to.be(5);
        expect(node.n).to.be.a('string');
        expect(node.n).to.be('boss');
        expect(node.v).to.be.a('string');
        expect(node.v).to.be('Bepi');
    });
});
