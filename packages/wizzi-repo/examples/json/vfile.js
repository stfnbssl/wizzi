/*
    ...
    utc time: Fri, 12 Apr 2024 14:01:12 GMT
*/
'use strict';
//
...
json.createJsonFsData(data_get_documents_folder1(), function(err, jsonFsData) {
    ...
    printValue("jsonFsData", stringify(jsonFsData, null, 2))
    var fsimpl = new JsonFsImpl();
    fsimpl.open({
        jsonFsData: jsonFsData
     }, function(err, docManager) {
        if (err) {
            console.log("[31m%s[0m", err);
            throw new Error(err.message);
        }
        printValue("fsimpl.jsonFs", stringify(fsimpl.jsonFs, null, 2))
        vfile(fsimpl, function(err, jsonVFS) {
            if (err) {
                console.log("[31m%s[0m", err);
                throw new Error(err.message);
            }
            jsonVFS.read('json:/root/folder1/index.html.ittf', function(err, content) {
                if (err) {
                    console.log("[31m%s[0m", err);
                    throw new Error(err.message);
                }
                console.log('readAsync', content);
                jsonVFS.write('json:/root/folder1/index.txt', 'Hello', function(err, result) {
                    if (err) {
                        console.log("[31m%s[0m", err);
                        throw new Error(err.message);
                    }
                    console.log('writeAsync', result);
                    jsonVFS.read('json:/root/folder1/index.txt', function(err, content) {
                        if (err) {
                            console.log("[31m%s[0m", err);
                            throw new Error(err.message);
                        }
                        console.log('readAsync 2', content);
                    })
                })
            })
        })
...
var VFile_Step_4 = function(step_callback) {
    heading1('EXAMPLE')
};
VFile_Step_4.__name = 'VFile_Step_4';
function heading1(text) {
    console.log('');
    console.log('*'.repeat(120));
    console.log('** level 0 - step 5 - VFile_Step_4 - ' + text);
    console.log('*'.repeat(120));
    console.log('');
}
