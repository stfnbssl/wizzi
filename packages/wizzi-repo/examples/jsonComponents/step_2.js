...
//
/**
     Examples: JsonComponents_Step_2
...
var JsonComponents_Step_2 = function(step_callback) {
    heading1('EXAMPLE')
    heading1('start');
    fSystem.fsFolderToPackiFiles(path.join(__dirname, 'packi'), function(err, packiFiles) {
        if (err) {
        ...
        repoIndex.JsonComponents.packiFilesToUtilVFileJson(packiFiles, function(err, jsonVFile) {
            ...
            jsonVFile.read('json:/alpha.js.ittf', function(err, content) {
                if (err) {
                    console.log("[31m%s[0m", err);
                    throw new Error(err.message);
                }
                printValue('alpha.js.ittf', content)
            })
        })
    })
};
JsonComponents_Step_2.__name = 'JsonComponents_Step_2';
function heading1(text) {
    console.log('');
    console.log('*'.repeat(120));
    console.log('** level 0 - step 1 - JsonComponents_Step_2 - ' + text);
    console.log('*'.repeat(120));
    console.log('');
}
function heading2(text) {
    ...
    console.log('   ','-- JsonComponents_Step_2 - ' + text);
    console.log('   ', '-'.repeat(100));
    console.log('');
}
...
module.exports = JsonComponents_Step_2;
if (typeof require != 'undefined' && require.main === module) {
    JsonComponents_Step_2();
}
