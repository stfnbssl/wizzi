/*
    ...
    package: wizzi-js@
    primary source IttfDocument: C:\My\wizzi\stfnbssl\wizzi\packages\wizzi-utils\.wizzi\examples\ittfScanner\build-ittf-graph.js.ittf
    utc time: Wed, 12 Jun 2024 06:21:07 GMT
*/
'use strict';
/**
     Example: ittfScanner_build_ittfGraph
...
var ittfScanner_build_ittfGraph = function(step_callback) {
    heading1('EXAMPLE')
    ...
    vfile(function(err, file) {
        ...
        file.getGlobbedFilesEx(path.join(folderPath, '**/*.ittf'), {
            ...
            console.log('ittfs\n', JSON.stringify(ittfs, null, 2), __filename);
            ittfGraph.createIttfDocumentGraphFrom(null, {
                ...
                root.analize(function(err, notUsed) {
                    ...
                    file.write(path.join(__dirname, 'outputs', 'build-ittf-graph', 'ittfFsNode_after_analize.json'), stringify(root, null, 2))
                    ...
                    ittfDocumentGraph.writeFile(path.join(__dirname, 'outputs', 'build-ittf-graph', 'ittfFsNode_wzpackage.json.ittf'), function(err, result) {
                        if (err) {
                            console.log("[31m%s[0m", err);
                            throw new Error(err.message);
                        }
                        console.log('calling root.setInfo', __filename);
                        root.setInfo(function(err, result) {
                            if (err) {
                                console.log("[31m%s[0m", err);
                                throw new Error(err.message);
                            }
                            ...
                            file.write(path.join(__dirname, 'outputs', 'build-ittf-graph', 'ittfFsNode_after_setInfo.json'), stringify(root, null, 2))
...
ittfScanner_build_ittfGraph.__name = 'ittfScanner_build_ittfGraph';
function heading1(text) {
    console.log('');
    console.log('*'.repeat(120));
    console.log('** level 0 - step 1 - ittfScanner_build_ittfGraph - ' + text);
    console.log('*'.repeat(120));
    console.log('');
}
function heading2(text) {
    ...
    console.log('   ','-- ittfScanner_build_ittfGraph - ' + text);
    console.log('   ', '-'.repeat(100));
    console.log('');
}
...
module.exports = ittfScanner_build_ittfGraph;
if (typeof require != 'undefined' && require.main === module) {
    ittfScanner_build_ittfGraph();
}