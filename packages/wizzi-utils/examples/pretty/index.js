/*
    ...
    package: wizzi-js@
    primary source IttfDocument: C:\My\wizzi\stfnbssl\wizzi\packages\wizzi-utils\.wizzi\examples\pretty\index.js.ittf
    utc time: Sat, 30 Mar 2024 12:44:02 GMT
*/
'use strict';
/**
     Example: pretty_Step_1
...
var pretty_Step_1 = function(step_callback) {
    heading1('EXAMPLE')
    ...
    function step_1() {
        ittfGraph.createIttfDocumentGraphFrom(path.join(__dirname, 'ittf', 'basic.sample.ittf'), {}, function(err, result) {
            ...
            pretty.prettifyIttfHtml(result, {}, function(err, ittfHtmlPretty) {
                if (err) {
                    console.log("[31m%s[0m", err);
                    throw new Error(err.message);
                }
                file.write(path.join(__dirname, 'outputs', 'basic.sample.pretty.html'), ittfHtmlPretty.prettyLines.join('\n'))
                step_2();
            })
        })
    }
    function step_2() {
        pretty.prettifyIttfHtml(path.join(__dirname, 'ittf', 'basic.sample.ittf'), {}, function(err, ittfHtmlPretty) {
            if (err) {
                console.log("[31m%s[0m", err);
                throw new Error(err.message);
            }
            console.log('step_2', ittfHtmlPretty);
            step_3();
        })
    }
    function step_3() {
        pretty.prettifyIttfHtmlFromString([
            'module', 
            '\t$include item'
        ].join('\n'), function(err, pretty) {
            if (err) {
...
pretty_Step_1.__name = 'pretty_Step_1';
function heading1(text) {
    console.log('');
    console.log('*'.repeat(120));
    console.log('** level 0 - step 1 - pretty_Step_1 - ' + text);
    console.log('*'.repeat(120));
    console.log('');
}
function heading2(text) {
    ...
    console.log('   ','-- pretty_Step_1 - ' + text);
    console.log('   ', '-'.repeat(100));
    console.log('');
}
