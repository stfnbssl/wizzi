...
export class MetaProductionController implements ControllerType {
    ...
    private postNewMeta = async (request: Request, response: Response) => {
        ...
        createMetaProduction((request.session as any).user.username, request.body.mp_name, request.body.mp_description, JSON.stringify(getPackiFiles('.wizzi/generate.wfjob.ittf'))).then((result: CRUDResult) => {
