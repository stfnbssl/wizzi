/*
    ...
    package: wizzi-js@
    primary source IttfDocument: C:\My\wizzi\stfnbssl\wizzi\packages\wizzi\.wizzi\examples\json\gen\step_1.js.ittf
    utc time: Thu, 18 Apr 2024 15:05:47 GMT
*/
'use strict';
/**
     Example: json_gen_step_1
...
var json_gen_step_1 = function(step_callback) {
    heading1('EXAMPLE')
    ...
    wizziIndex.JsonComponents.createJsonFs(documents, function(err, jsonFs) {
        if (err) {
        ...
        wizziIndex.jsonFactory(options, function(err, wf) {
            if (err) {
                console.log("[31m%s[0m", 'Test error >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>');
                console.log("[31m%s[0m", 'err', err);
                throw new Error(err.message);
            }
            wf.generateFolderArtifacts(packiFilePrefix + 'src', {
                modelRequestContext: {
                ...
                printObject('generateFolderArtifacts.result', result)
                wf.fileService.getFiles(packiFilePrefix + 'out', {
                    deep: true, 
                    documentContent: true
                 }, function(err, files) {
                    if (err) {
                    ...
                    printObject('files.result', files)
                    const packiFiles = {};
                    var i, i_items=files, i_len=files.length, file;
                    for (i=0; i<i_len; i++) {
                        file = files[i];
                    ...
                    printObject('files.packiFiles', packiFiles)
                })
            })
        })
    })
};
json_gen_step_1.__name = 'json_gen_step_1';
function heading1(text) {
    console.log('');
    console.log('*'.repeat(120));
    console.log('** level 0 - step 1 - json_gen_step_1 - ' + text);
    console.log('*'.repeat(120));
    console.log('');
}
function heading2(text) {
    ...
    console.log('   ','-- json_gen_step_1 - ' + text);
    console.log('   ', '-'.repeat(100));
    console.log('');
}
...
module.exports = json_gen_step_1;
