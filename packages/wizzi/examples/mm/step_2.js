/*
    ...
    package: wizzi-js@
    primary source IttfDocument: C:\My\wizzi\stfnbssl\wizzi\packages\wizzi\.wizzi\examples\mm\step_2.js.ittf
...
var mm_step_2 = function(step_callback) {
    heading1('EXAMPLE')
    
    const metaDemoIttfFolder = "C:/My/wizzi/stfnbssl/wizzi.metas/packages/wizzi.meta.demo/ittf";
    
    wizziUtils.fSystem.fsFolderToPackiFiles(metaDemoIttfFolder, function(err, packiFiles) {
        if (err) {
            console.log("[31m%s[0m", 'Test error >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>');
            console.log("[31m%s[0m", 'err', err);
            throw new Error(err.message);
        }
        printValue('packiFiles', stringify(packiFiles, null, 2))
        inmemoryMetaPlugin.createMetaPlugin({
            ...
            gmp.getMetaCategoryStarter({}, (err, result) => {
            
                if (err) {
                    console.log("[31m%s[0m", err);
                    return callback(err);
                }
                printValue('getMetaCategoryStarter', stringify(result, null, 2))
            }
            )
            gmp.getMetaContextDefsStarter({
                metaCtx: {
                    useDemoJs: true, 
                    demoJs: {
                        name: "Stefano"
                ...
                printValue('getMetaContextDefsStarter', stringify(result, null, 2))
            }
            )
            gmp.getMetaProductionStarter({
                metaCtx: {
                    useDemoJs: true, 
                    demoJs: {
                        name: "Stefano"
                ...
                printValue('getMetaProductionStarter', stringify(result, null, 2))
            }
            )
        }
        )
...
mm_step_2.__name = 'mm_step_2';
function heading1(text) {
    console.log('');
    console.log('*'.repeat(120));
    console.log('** level 0 - step 1 - mm_step_2 - ' + text);
    console.log('*'.repeat(120));
    console.log('');
}
