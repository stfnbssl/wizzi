...
var mm_step_1 = function(step_callback) {
    ...
    createMetasManager({}, (err, mm) => {
        ...
        mm.getMetaProduction("eslint", (err, metaProduction) => {
        
            if (err) {
                console.log("[31m%s[0m", err);
                return ;
            }
            console.log('metaProduction', metaProduction, __filename);
        }
        )
    }
    )
...
function heading1(text) {
    ...
    console.log('** level 0 - step 1 - mm_step_1 - ' + text);
    console.log('*'.repeat(120));
    console.log('');
}
