/*
    ...
    package: wizzi-js@
    primary source IttfDocument: C:\My\wizzi\stfnbssl\wizzi\packages\wizzi\.wizzi\examples\wf\gen\step_1.js.ittf
...
var wf_gen_step_1 = function(step_callback) {
    heading1('EXAMPLE')
    wizziIndex.createFactory({
        ...
        var test1Path = path.join(__dirname, 'ittf', 'test1.ittf.ittf');
        wf.loadModelAndGenerateArtifact(test1Path, {
            modelRequestContext: {
                metaCtx: {
                    name: "Hello"
                 }
             }
         }, 'ittf/document', function(err, artifactText) {
            if (err) {
                console.log("[31m%s[0m", 'Test error >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>');
                console.log("[31m%s[0m", 'err', err);
                throw new Error(err.message);
            }
            printValue('Generated Artifact: ittf/test1', artifactText)
        })
    })
};
wf_gen_step_1.__name = 'wf_gen_step_1';
function heading1(text) {
    console.log('');
    console.log('*'.repeat(120));
    console.log('** level 0 - step 1 - wf_gen_step_1 - ' + text);
    console.log('*'.repeat(120));
    console.log('');
}
function heading2(text) {
    ...
    console.log('   ','-- wf_gen_step_1 - ' + text);
    console.log('   ', '-'.repeat(100));
    console.log('');
}
