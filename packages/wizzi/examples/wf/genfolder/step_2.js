/*
    ...
    package: wizzi-js@
    primary source IttfDocument: C:\My\wizzi\stfnbssl\wizzi\packages\wizzi\.wizzi\examples\wf\genfolder\step_2.js.ittf
    utc time: Wed, 15 May 2024 16:05:48 GMT
*/
'use strict';
/**
     Example: genfolder_step_2
...
const packiFilePrefixExtract = wizziIndex.constantss.packiFilePrefixExtract;
...
var genfolder_step_2 = function(step_callback) {
    heading1('EXAMPLE')
    var genFolderPathFs = path.join(__dirname, 'ittf', 'folder1');
    var destFolderPathFs = path.join(__dirname, 'ittf', 'folder1_generated');
    var friendsArray = [
        'arthur', 
        'mary'
    ];
    createPackifilesFromFs(genFolderPathFs, (err, folder1PackiFiles) => {
    
        if (err) {
            console.log("[31m%s[0m", err);
            return callback(err);
        }
        printValue('folder1PackiFiles', stringify(folder1PackiFiles, null, 2))
        createJsonWizziFactoryAndJsonFs(folder1PackiFiles, function(err, wf_and_jsonFs) {
            if (err) {
            ...
            printValue('folder1PackiFiles_jsonFs', stringify(jsonFs, null, 2))
            var genFolderPath = 'root';
            var genFolderDest = 'result';
            wf.generateFolderArtifacts(packiFilePrefix + genFolderPath, {
                modelRequestContext: {
                    metaCtx: {
                        name: "Hello"
                     }, 
                ...
                copyInclude: ['*'], 
                copyExclude: []
             }, function(err, generatedFilePaths) {
                if (err) {
                    console.log("[31m%s[0m", 'Test error >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>');
                    console.log("[31m%s[0m", 'err', err);
                    throw new Error(err.message);
                }
                printValue('generatedFilePaths', generatedFilePaths)
                printValue('folder1PackiFiles_after_gen_jsonFs', stringify(jsonFs, null, 2))
                packiUtils.jsonFsToPackiFiles(jsonFs, genFolderDest, (err, generatedPackiFiles) => {
                
                    if (err) {
                        console.log("[31m%s[0m", err);
                        return callback(err);
                    }
                    printValue('generatedPackiFiles', generatedPackiFiles)
                    writePackifiles(destFolderPathFs, generatedPackiFiles)
                }
                )
            })
        })
...
genfolder_step_2.__name = 'genfolder_step_2';
function heading1(text) {
    console.log('');
    console.log('*'.repeat(120));
    console.log('** level 0 - step 1 - genfolder_step_2 - ' + text);
    console.log('*'.repeat(120));
    console.log('');
}
function heading2(text) {
    ...
    console.log('   ','-- genfolder_step_2 - ' + text);
    console.log('   ', '-'.repeat(100));
    console.log('');
}
...
module.exports = genfolder_step_2;
