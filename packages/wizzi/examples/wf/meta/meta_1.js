...
var wf_meta_meta_1 = function(step_callback) {
    heading1('EXAMPLE')
    
    const metaDemoIttfFolder = "C:/My/wizzi/stfnbssl/wizzi.metas/packages/wizzi.meta.demo/ittf";
    
    wizziUtils.fSystem.fsFolderToPackiFiles(metaDemoIttfFolder, function(err, metaDemoPackiFiles) {
        if (err) {
        ...
        createJsonWizziFactoryAndJsonFsWithOptions({}, {
            items: [
                './wizzi.plugin.html/index', 
                './wizzi.plugin.js/index', 
                './wizzi.plugin.css/index', 
         ...
         }, function(err, wf_and_jsonFs) {
            if (err) {
                console.log("[31m%s[0m", 'Test error >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>');
                console.log("[31m%s[0m", 'err', err);
                throw new Error(err.message);
            }
            wf_and_jsonFs.wf.getMetaParameters({
                metaCtx: {
                    useDemoJs: true, 
                    useDemoHtml: true
                 }
             }, function(err, metaParameters) {
                if (err) {
                    console.log("[31m%s[0m", 'Test error >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>');
                    console.log("[31m%s[0m", 'err', err);
                    throw new Error(err.message);
...
wf_meta_meta_1.__name = 'wf_meta_meta_1';
function heading1(text) {
    console.log('');
    console.log('*'.repeat(120));
    console.log('** level 0 - step 3 - wf_meta_meta_1 - ' + text);
    console.log('*'.repeat(120));
    console.log('');
}
function heading2(text) {
    ...
    console.log('   ','-- wf_meta_meta_1 - ' + text);
    console.log('   ', '-'.repeat(100));
    console.log('');
}
