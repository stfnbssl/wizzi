/*
    ...
    package: wizzi-js@
    primary source IttfDocument: C:\My\wizzi\stfnbssl\wizzi\packages\wizzi\.wizzi\examples\wf\transform\step_1.js.ittf
    utc time: Thu, 18 Apr 2024 15:05:47 GMT
*/
'use strict';
/**
     Example: wf_transform_step_1
...
var wf_transform_step_1 = function(step_callback) {
    heading1('EXAMPLE')
    var csPath = path.join(__dirname, 'ittf', 'cheatsheet.ittf.ittf');
    createWizziFactory({}, function(err, wf) {
        ...
        wf.loadAndTransformModel(csPath, {
            modelRequestContext: {
                
             }
         }, 'ittf/cheatsheet', function(err, wizziModel) {
            if (err) {
                console.log("[31m%s[0m", 'Test error >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>');
                console.log("[31m%s[0m", 'err', err);
                throw new Error(err.message);
            }
            printValue('transformed cheatsheet', stringify(wizziModel, null, 2))
        })
    })
};
wf_transform_step_1.__name = 'wf_transform_step_1';
function heading1(text) {
    console.log('');
    console.log('*'.repeat(120));
    console.log('** level 0 - step 1 - wf_transform_step_1 - ' + text);
    console.log('*'.repeat(120));
    console.log('');
}
function heading2(text) {
    ...
    console.log('   ','-- wf_transform_step_1 - ' + text);
    console.log('   ', '-'.repeat(100));
    console.log('');
}
...
module.exports = wf_transform_step_1;
