/*
    ...
    package: wizzi-js@
    primary source IttfDocument: C:\My\wizzi\stfnbssl\wizzi\packages\wizzi\.wizzi\examples\wf\wizzify\step_2.js.ittf
    utc time: Thu, 18 Apr 2024 15:05:47 GMT
*/
'use strict';
/**
     Example: wf_wizzify_step_1
...
var wf_wizzify_step_1 = function(step_callback) {
    ...
    createWizziFactory({}, function(err, wf) {
        ...
        wf.getCodeAST(htmlSourcePath, 'html', function(err, astObject) {
            if (err) {
                console.log("[31m%s[0m", 'Test error >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>');
                console.log("[31m%s[0m", 'err', err);
                throw new Error(err.message);
            }
            printValue('html AST object', astObject)
            wf.getCodeAST(cssSourcePath, 'css', function(err, astObject) {
                if (err) {
                    console.log("[31m%s[0m", 'Test error >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>');
                    console.log("[31m%s[0m", 'err', err);
                    throw new Error(err.message);
                }
                printValue('css AST object', astObject)
                wf.getCodeAST(svgSourcePath, 'svg', function(err, astObject) {
                    if (err) {
                        console.log("[31m%s[0m", 'Test error >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>');
                        console.log("[31m%s[0m", 'err', err);
                        throw new Error(err.message);
                    }
                    printValue('svg AST object', astObject)
                    wf.getCodeAST(jsSourcePath, 'js', function(err, astObject) {
                        if (err) {
                            console.log("[31m%s[0m", 'Test error >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>');
                            console.log("[31m%s[0m", 'err', err);
                            throw new Error(err.message);
                        }
                        printValue('js AST object', astObject)
                        wf.getCodeAST(graphqlSourcePath, 'graphql', function(err, astObject) {
                            if (err) {
                                console.log("[31m%s[0m", 'Test error >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>');
                                console.log("[31m%s[0m", 'err', err);
                                throw new Error(err.message);
                            }
                            printValue('graphql AST object', astObject)
                            wf.getCodeAST(tsSourcePath, 'ts', function(err, astObject) {
                                if (err) {
                                    console.log("[31m%s[0m", 'Test error >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>');
                                    console.log("[31m%s[0m", 'err', err);
                                    throw new Error(err.message);
                                }
                                printValue('ts AST object', astObject)
                                wf.getCodeAST(jsonSourcePath, 'json', function(err, astObject) {
                                    if (err) {
                                        console.log("[31m%s[0m", 'Test error >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>');
                                        console.log("[31m%s[0m", 'err', err);
                                        throw new Error(err.message);
                                    }
                                    printValue('json AST object', astObject)
                                })
                            })
                        })
                    })
...
wf_wizzify_step_1.__name = 'wf_wizzify_step_1';
function heading1(text) {
    console.log('');
    console.log('*'.repeat(120));
    console.log('** level 0 - step 1 - wf_wizzify_step_1 - ' + text);
    console.log('*'.repeat(120));
    console.log('');
}
function heading2(text) {
    ...
    console.log('   ','-- wf_wizzify_step_1 - ' + text);
    console.log('   ', '-'.repeat(100));
    console.log('');
}
...
module.exports = wf_wizzify_step_1;
