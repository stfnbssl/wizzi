/*
    ...
    package: wizzi-js@
    primary source IttfDocument: C:\My\wizzi\stfnbssl\wizzi\packages\wizzi\.wizzi\examples\wf\wizzify\step_4.js.ittf
    utc time: Thu, 18 Apr 2024 15:05:47 GMT
*/
'use strict';
/**
     Example: wf_wizzify_step_4
...
var wf_wizzify_step_4 = function(step_callback) {
    heading1('EXAMPLE')
    var yamlSourcePath = path.join(__dirname, 'data', 'comparable_1.yaml');
    var yamlSource = file.read(yamlSourcePath);
    createWizziFactory({}, function(err, wf) {
        ...
        wf.wizzifyWithCompare(yamlSource, 'yaml', 'yaml/document', function(err, result) {
            if (err) {
                console.log("[31m%s[0m", 'Test error >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>');
                console.log("[31m%s[0m", 'err', err);
                throw new Error(err.message);
            }
            printValue('compare yaml result', result)
        })
    })
};
wf_wizzify_step_4.__name = 'wf_wizzify_step_4';
function heading1(text) {
    console.log('');
    console.log('*'.repeat(120));
    console.log('** level 0 - step 1 - wf_wizzify_step_4 - ' + text);
    console.log('*'.repeat(120));
    console.log('');
}
function heading2(text) {
    ...
    console.log('   ','-- wf_wizzify_step_4 - ' + text);
    console.log('   ', '-'.repeat(100));
    console.log('');
}
...
module.exports = wf_wizzify_step_4;
