/*
    ...
    primary source IttfDocument: C:\My\wizzi\stfnbssl\wizzi\packages\wizzi\.wizzi\lib\production\artifact\genContext.js.ittf
...
var GenContext = (function () {
    function GenContext(genconfig, parent) {
        ...
        if (verify.isNullOrUndefined(parent) === false) {
            if (verify.isObject(parent) === false) {
                throw new Error(error(
                    'InvalidArgument', 'ctor', { parameter: 'parent', message: 'The parent parameter must be an object. Received: ' + parent }
                ));
            }
        }
        //
    ...
    GenContext.prototype.w = function(text) {
        ...
        if (verify.isString(text)) {
            ...
            if (this.forceInline || this.forceInlineExtra) {
                this.block.write(this.doEscape(iptext + ' '))
            }
            else {
                this.block.w(this.doEscape(iptext))
            }
        }
        else {
            if (this.forceInline || this.forceInlineExtra) {
    ...
    GenContext.prototype.write = function(text) {
        ...
        this.block.write(this.doEscape(text.indexOf('{') > -1 ? interpolate(text, this.values) : text))
        this.isEmpty = false;
    ...
    GenContext.prototype.writeAligned = function(text) {
        ...
        for (i=0; i<i_len; i++) {
            ...
            if (i < i_len - 1 || line.length > 0) {
                this.block.w(this.doEscape(line.indexOf('{') > -1 ? interpolate(line, this.values) : line))
            }
        }
        this.isEmpty = false;
        this.lineLength = 0;
    ...
    GenContext.prototype.wSameLine = function(text) {
        this.block.wSameLine(this.doEscape(text))
        this.isEmpty = false;
        this.lineLength += text.length;
    }
    ...
    GenContext.prototype.isLineEmpty = function() {
    ...
    GenContext.prototype.artifactGenerationError = function(message, location, node) {
        ...
        if (this.parent) {
            this.parent.artifactGenerationErrors.push(error);
        }
        else {
    ...
    GenContext.prototype.addEscape = function(ch) {
        this.escapes[ch] = ch;
        this.hasEscapes = true;
    }
    GenContext.prototype.removeEscape = function(ch) {
        delete this.escapes[ch]
        this.hasEscapes = Object.keys(this.escapes).length > 0;
    }
    GenContext.prototype.doEscape = function(text) {
        ...
        for (var i=0; i<text.length; i++) {
            ret.push(this.doEscapeChar(text[i]))
        }
        return ret.join('');
    }
    GenContext.prototype.doEscapeChar = function(ch) {
    ...
    GenContext.prototype.createChildGenContext = function() {
