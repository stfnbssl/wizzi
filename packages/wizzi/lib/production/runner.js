...
var Runner = (function () {
    ...
    Runner.prototype._runOnContextInfos = function(contextInfos, callback) {
        if (this.productionStep.isCodeWriteArtifact()) {
            ...
            return callback(this.error('Artifact is in an invalid state: ' + util.inspect(this.productionStep.getState(), {depth: 2})));
        }
    }
    Runner.prototype._runOnModelInfo = function(modelInfo, callback) {
        ...
        if (this.productionStep.isWizziModelDomArtifact()) {
            ...
            return callback(this.error('The production step is in an invalid state: ' + util.inspect(this.productionStep.getState(), {depth: 2})));
        }
    }
    Runner.prototype._runOnIttfDocumentProduction = function(ittfDocumentProduction, callback) {
        if (ittfDocumentProduction.productionActionType == "loadModelAndGenerateArtifact") {
            ...
            return callback(this.error('Invalid `ittfDocumentProduction.productionActionType`. The production step is in an invalid state: ' + util.inspect(this.productionStep.getState(), {depth: 2})));
        }
    }
    Runner.prototype.runWizziModelDomArtifact = function(modelInfo, callback) {
        ...
        modelInfo.getModelInfos({
            ...
            if (modelInfos.length != 1) {
                return callback(this.error('Expected exactly one wizzi.model.modelInfo. Received: ' + modelInfos.length));
            }
            ...
            if (basenameParts.length != 3) {
                return callback(this.error("The name of the source 'wfschema' ittf document must be in the format '<schema-name>.wfschema.ittf'. Received: " + path.basename(wfschemaIttfDocumentUri)));
            }
