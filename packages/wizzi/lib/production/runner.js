...
/**
     ...
     Input: an ProductionStep (constructor injected).
...
var Runner = (function () {
    ...
    Runner.prototype._runOnContextInfos = function(contextInfos, callback) {
        if (this.productionStep.isCodeWriteArtifact()) {
            ...
            return callback(error('999', '_runOnContextInfos', 'Artifact is in an invalid state: ' + util.inspect(this.productionStep, {depth: 2})));
        }
    }
    Runner.prototype._runOnModelInfo = function(modelInfo, callback) {
        var gen = this.productionStep.gen;
        if (this.productionStep.isWizziModelTypesArtifact()) {
            ...
            return callback(error('999', '_runOnModelInfo', 'Artifact is in an invalid state: ' + util.inspect(this.productionStep, {depth: 2})));
        }
    }
    Runner.prototype.runWizziModelTypesArtifact = function(modelInfo, callback) {
        var that = this;
        log.warn('runWizziModelTypesArtifact ' + this.productionStep.name);
        ...
        modelInfo.getModelInfos({
            ...
            AsyncWizziModelTypesRunner.run(wmtRequest, function(err, result) {
                ...
                callback(null, that.productionStep);
            })
        })
    }
        ...
        modelInfo.getModelInfos({
            ...
            modelInfo.getArtifactGenerator(gen.generator, function(err, generator) {
                ...
                AsyncModelLoader.loadMany(modelInfos, function(err, mainSourceModels) {
                    ...
                    AsyncArtifactGenerator.generate(that.productionStep, generator, modelInfos, mainSourceModels, function(err, notUsed) {
                        if (err) {
                            return callback(err);
                        }
                        callback(null, that.productionStep);
        ...
        modelInfo.getModelInfos({
            ...
            modelInfo.getArtifactGenerator(gen.generator, function(err, generator) {
                ...
                AsyncModelLoader.loadMany(modelInfos, function(err, mainSourceModelsOfModelCollection) {
                    ...
                    AsyncArtifactGenerator.generateModelCollection(that.productionStep, generator, modelInfos, mainSourceModelsOfModelCollection, function(err, result) {
                        if (err) {
                            return callback(err);
                        }
    ...
    Runner.prototype.runCodeWriteArtifact = function(contextInfos, gen, callback) {
        ...
        modelInfo.getArtifactGenerator(gen.generator, function(err, generator) {
            ...
            AsyncModelLoader.loadMany(contextInfos, function(err, mainSourceModels) {
                ...
                AsyncArtifactGenerator.generateCodeWrite(that.productionStep, generator, mainSourceModels, function(err, notUsed) {
                    if (err) {
                        return callback(err);
                    }
                    callback(null, that.productionStep);
                ...
                modelInfo.getFiles({
                    ...
                    for (i=0; i<i_len; i++) {
                        ...
                        var genContext = new GenContext({
                            ...
                            options: that.productionStep.options, 
                            pman: modelInfo.productionManager()
                         });
                        genContext.writeFile(fileInfo.fullpath)
                        that.productionStep.addGenContext(genContext);
                    }
                    return callback(null, that.productionStep);
                })
            }
            else {
                var genContext = new GenContext({
