...
var WzjobLoader = (function () {
    ...
    WzjobLoader.prototype.load = function(wfjobRequest, callback) {
        this.seenWzjobRequests.push(wfjobRequest);
        ...
        var that = this;
        this.productionManager.loadWzjob(ittfDocumentUri, wfjobRequest.context || {}, function(err, wzjobModel) {
            if (err) {
                return callback(err);
            }
    ...
    WzjobLoader.prototype.processWzjobModel = function(wzjobModel) {
        /**
             ...
             Creates a wizzi.productionManager.productionStepConfig object for each artifact production request.
             Accumulates the requests calling the methods:
             . addPendingWzjobRequest(productionStepConfig)
             . addProductionStepConfig(productionStepConfig)
        ...
        */
        ...
        var i, i_items=wzjobModel.productions, i_len=wzjobModel.productions.length, wfjobProductionElement;
        for (i=0; i<i_len; i++) {
            wfjobProductionElement = wzjobModel.productions[i];
            // loog 'wizzi.wzjobLoader.processWzjobModel.production', wfjobProductionElement.wzName, util.inspect(wfjobProductionElement, { depth: 2 })
            var j, j_items=wfjobProductionElement.lines, j_len=wfjobProductionElement.lines.length, wzjobLineElement;
            for (j=0; j<j_len; j++) {
                wzjobLineElement = wfjobProductionElement.lines[j];
                // loog 'wizzi.wzjobLoader.processWzjobModel.line', wzjobLineElement.wzName, util.inspect(wzjobLineElement, { depth: null })
                var k, k_items=wzjobLineElement.artifacts, k_len=wzjobLineElement.artifacts.length, wfjobArtifactElement;
                for (k=0; k<k_len; k++) {
                    wfjobArtifactElement = wzjobLineElement.artifacts[k];
                    // loog 'wizzi.wzjobLoader.processWzjobModel.artifact', util.inspect(wfjobArtifactElement, { depth: 1 })
                    ...
                    */
                    if (verify.isEmpty(wfjobArtifactElement.src)) {
                        productionStepConfig = {
                            ...
                            dest: {
                                ...
                                folder: wzjobLineElement.destFolder || wfjobProductionElement.destFolder, 
                    ...
                    // loog 'WizziJob.getProductionStepConfigs.art', wfjobArtifactElement.wzName, wzjobLineElement.srcFolder, wfjobArtifactElement.src, wfjobArtifactElement.schema
                    else {
                        productionStepConfig = {
                            name: wfjobArtifactElement.wzName, 
                            options: {}, 
                            ...
                            isWzJob: wfjobArtifactElement.isWzJob, 
                    ...
                    if (wfjobArtifactElement.isWzJob) {
...
*/
function AsyncRecurseWzjobLoad(wfjobRequest, callback) {
    ...
    if (verify.isObject(wfjobRequest) === false) {
        return callback(error(
            'InvalidArgument', 'AsyncRecurseWzjobLoad', { parameter: 'wfjobRequest', message: 'The wfjobRequest parameter must be an object. Received: ' + wfjobRequest }
        ));
    }
    if (verify.isObject(wfjobRequest.wzjobLoader) === false) {
        return callback(error(
            'InvalidArgument', 'AsyncRecurseWzjobLoad', { parameter: 'wfjobRequest.wzjobLoader', message: 'The wfjobRequest.wzjobLoader parameter must be an object. Received: ' + wfjobRequest.wzjobLoader }
        ));
    }
    console.log('=');
    console.log('= ========= wizzi.AsyncRecurseWzjobLoad. Start. wfjobRequest.model : ', wfjobRequest.model);
    console.log('=');
    var wzjobLoader = wfjobRequest.wzjobLoader;
    wzjobLoader.load(wfjobRequest, function(err, result) {
        if (err) {
            return callback(err);
        }
        console.log('=');
